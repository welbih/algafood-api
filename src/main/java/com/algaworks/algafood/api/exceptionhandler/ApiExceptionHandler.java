...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleHttpMessageNotReadable(HttpMessageNotReadableException ex,
        ...
        if(rootCause instanceof InvalidFormatException) {
            return handleInvalidFormatException((InvalidFormatException) rootCause, headers, status, request);
                    ...
                    || rootCause instanceof UnrecognizedPropertyException) {
    ...
    private ResponseEntity<Object> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException ex,
        ...
        var detail = String
                .format("O parâmetro de URL '%s' recebeu o valor '%s', que é um tipo inválido. Corrija e informe um valor compatível com o tipo '%s' "
                        , ex.getName(), ex.getValue(), ex.getRequiredType().getSimpleName());

        var problem = createProblemBuilder(status, problemType, detail)
                .userMessage("Você inseriu um valor inválido. Corrija o valor e tente novamente.")
                .build();
    ...
    private ResponseEntity<Object> handlePropertyBindingException(PropertyBindingException ex,
        ...
        ProblemType problemType = ProblemType.MENSAGEM_INCOMPREENSIVEL;
        String detail = String.format("O atributo '%s' é inválido. ", path);
    ...
    public ResponseEntity<?> handleEntidadeNaoEncontradoException(NegocioException e, WebRequest request) {
    ...
    protected ResponseEntity<Object> handleExceptionInternal(Exception ex, Object body, HttpHeaders headers,
        ...
        if (isNull(body))
            body = buildProblem(status.getReasonPhrase(), status.value());
        else if (body instanceof String)
            body = buildProblem((String) body, status.value());

        return super.handleExceptionInternal(ex, body, headers, status, request);
    }

    private Problem buildProblem(String title, Integer status) {
